...
public interface GroupCategoryRepository extends JpaRepository<GroupCategory, Long>, JpaSpecificationExecutor<GroupCategory> {
    ...
    @org.springframework.data.jpa.repository.Query(
        "SELECT COUNT(g) > 0 FROM GroupCategory g WHERE g.paramName = :paramName " +
    ...
    boolean existsOverlapping(
            @org.springframework.data.repository.query.Param("paramName") String paramName,
            @org.springframework.data.repository.query.Param("paramType") String paramType,
            @org.springframework.data.repository.query.Param("effectiveDate") java.time.LocalDateTime effectiveDate,
            @org.springframework.data.repository.query.Param("endEffectiveDate") java.time.LocalDateTime endEffectiveDate,
